...
CREATE INDEX idx_homepage_banners_status_times ON homepage_banners(status, start_at, end_at);
CREATE TRIGGER trg_homepage_banners_updated BEFORE UPDATE ON homepage_banners FOR EACH ROW EXECUTE FUNCTION set_updated_at();

ALTER TABLE homepage_banners ENABLE ROW LEVEL SECURITY;
CREATE POLICY "public_read_active_banners" ON homepage_banners FOR SELECT USING (status = 'active' AND (start_at IS NULL OR start_at <= NOW()) AND (end_at IS NULL OR end_at >= NOW()));
CREATE POLICY "admin_manage_banners" ON homepage_banners FOR ALL USING (
  EXISTS (SELECT 1 FROM users WHERE uid = (current_setting('request.jwt.claims', true)::json->>'sub')::uuid AND role='admin')
);
